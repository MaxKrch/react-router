...
const PostFeed = () => {
  ...
  const { state: posts, setPost } = usePostActions()

  useEffect(() => {
    if (loaderPosts.status === STATUS.SUCCESS && loaderPosts.data)
      setPost(loaderPosts.data)
  }, [loaderPosts, setPost])
  ...
  return (
    <div className="flex flex-col gap-4 w-full min-h-full p-2">
      ...
      <ul className="flex flex-col gap-2">
        {posts.map((post) => (
          <li key={post.id}>
            <Post post={post} onClick={handleClickPost} />
          </li>
        ))}
      </ul>
    </div>
