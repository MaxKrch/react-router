...
const usePostForm = ({
    ...
    const handleSubmit = useCallback(async () => {
        ...
        currentSendingId.current = initialData
            ? initialData.id 
            : Date.now()
        ...
        const action = mode === 'create'
            ...
            : generatePathWithId(ROUTES.SOCIAL_NETWORK.UPDATE_POST, Number(tempPost.id))

        await fetcher.submit(formData, {
            method: 'post',
            action
        })
    }, [formRef, initialData, addPost, updatePost, fetcher, mode]);
